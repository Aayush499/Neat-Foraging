...
def generate_prefix():
    global obstacles, particles, generations, movement_type, network_type, sub, ricochet, obstacle_type, seeded, o_switch, decay_factor, pheromone_receptor, collision_threshold, time_constant, teleport, num_sensors, food_calibration, fitness_criterion, stagnation
    if SPARSE_REWARD:
        rew = "Sparse"
    else:
        rew = "Dense"
    ...
    s= f"{sub}-F{particles}-G{generations}-N{network_type}-OS{o_switch}-D{decay_factor}-PR_{pheromone_receptor}-CT_{collision_threshold}-Movement_{movement_type}-Time_{time_constant}-Reward_{rew}"
...
class ForageTask:
    ...
    def calculate_fitness(self, game_info):
        ...
        if extra_sparse:
            ...
            waste = max(0.0, self.game.total_distance_travelled - self.game.optimal_distance)
            self.genome.fitness += game_info.score 
...
def run_neat(config):
    global obstacles, particles, generations, movement_type, network_type, sub, use_checkpoint, endless, storage_dir
    ...
    txt_filename_general = os.path.join(checkpoint_subdir, 'general_info.txt')
    ...
    p.add_reporter(neat.FileReporter(os.path.join(checkpoint_subdir, "neat_stdout.log"), True))
    p.add_reporter(neat.StdOutReporter(True))
    ...
    with open(txt_filename_general, 'w') as f:

        f.write(f"Network Type: {network_type}\n")
        f.write(f"Total Generations: {generations}\n")
        f.write(f"Number of Runs per Genome: {NUM_RUNS}\n")
        f.write(f"Movement Type: {movement_type}\n")
        f.write(f"Particles: {particles}\n")
        f.write(f"Pheromone Receptor: {pheromone_receptor}\n")
        f.write(f"Collision Threshold: {collision_threshold}\n")
        f.write(f"Time Constant: {time_constant}\n")
        f.write(f"Sparse Reward: {SPARSE_REWARD}\n")
        f.write(f"Extra Sparse Reward: {extra_sparse}\n")
        f.write(f"Decay Factor: {decay_factor}\n")
        f.write(f'Discount Factor: {discount_factor}\n')
        f.write(f"Optimal Distance Constraint: {distance_constraint}\n")
        f.write(f"Best_fitness_overall: {max(best_fitness)}\n")
...
if __name__ == '__main__':
    ...
    node_addition_rate = args.node_addition_rate
    node_deletion_rate = args.node_deletion_rate
    ...
    if SPARSE_REWARD:
        ...
        cfg['NEAT']['fitness_threshold'] = '150'

    cfg['DefaultStagnation']['max_stagnation'] = str(stagnation)
    cfg['DefaultGenome']['conn_add_prob'] = str(connection_addition_rate)
    cfg['DefaultGenome']['conn_delete_prob'] = str(connection_deletion_rate)
    cfg['DefaultGenome']['node_add_prob'] = str(node_addition_rate)
    cfg['DefaultGenome']['node_delete_prob'] = str(node_deletion_rate)
    cfg['DefaultReproduction']['elitism'] = str(elitism)
    cfg['DefaultGenome']['initial_connection'] = args.initial_connection
    cfg['DefaultGenome']['weight_mutate_power'] = str(weight_mutate_power)
    ...
    if manual_mode:
        ...
        if parameter_print:
            ...
            print(f"Orientation Switching: {o_switch}")
            print(f"Pheromone Receptor: {pheromone_receptor}")
            print(f"Carrying Food Receptor: {carrying_food_receptor}")
            print(f"Nest Receptor: {nest_receptor}")
            print(f"Collision Threshold: {collision_threshold}")
            print(f"Time Constant: {time_constant}")
            print(f"Distance Constraint: {distance_constraint}")
            print(f"Sparse Reward: {SPARSE_REWARD}")
            print(f"Extra Sparse: {extra_sparse}")
            print(f"Fitness Criterion: {fitness_criterion}")
            print(f"Stagnation: {stagnation}")
            print(f"Connection Addition Rate: {connection_addition_rate}")
            print(f"Connection Deletion Rate: {connection_deletion_rate}")
            print(f"Node Addition Rate: {node_addition_rate}")
            print(f"Node Deletion Rate: {node_deletion_rate}")
            print(f"Weight Mutate Power: {weight_mutate_power}")
