#!/bin/bash
#SBATCH --job-name=neat_main_baseline2 (mild mutations 0.3, 0.05)
#SBATCH --ntasks=1
...
case "$SLURM_ARRAY_TASK_ID" in
  1)
    python3 main_parallel.py $COMMON_ARGS \
      ...
      --storage_dir baseline2/recursive_no_drop
    ;;
  2)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_drop_1 \
      ...
      --storage_dir baseline2/recursive_dropper
    ;;
  3)
    python3 main_parallel.py $COMMON_ARGS \
      --sub ff_drop_1 \
      --network ff \
      --pheromone_receptor true \
      --storage_dir baseline2/ff_dropper
    ;;
  4)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_no_drop_2 \
