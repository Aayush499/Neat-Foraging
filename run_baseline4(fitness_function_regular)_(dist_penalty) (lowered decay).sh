#!/bin/bash
#SBATCH --job-name=neat_main_baseline4-mild-mutations-0.3-0.05_regular_fitness__dist_penalty_lowered_decay
#SBATCH --ntasks=1
...
COMMON_ARGS="--particles 3 --movement_type holonomic \
             ...
             --time_constant 200 \
             --sparse_reward false --extra_sparse false \
...
case "$SLURM_ARRAY_TASK_ID" in
  1)
    python3 main_parallel.py $COMMON_ARGS \
      ...
      --storage_dir baseline4_regular_fitness_dist_penalty_lowered_decay/recursive_no_drop
    ;;
  2)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_drop_1 \
      ...
      --storage_dir baseline4_regular_fitness_dist_penalty_lowered_decay/recursive_dropper
    ;;
  3)
    python3 main_parallel.py $COMMON_ARGS \
      --sub ff_drop_1 \
      --network ff \
      --pheromone_receptor true \
      --storage_dir baseline4_regular_fitness_dist_penalty_lowered_decay/ff_dropper
    ;;
  4)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_no_drop_2 \
