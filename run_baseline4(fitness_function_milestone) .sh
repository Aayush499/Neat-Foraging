#!/bin/bash
#SBATCH --job-name=neat_main_baseline4-mild-mutations-0.3-0.05_milestone_fitness__no_dist_penalty
#SBATCH --ntasks=1
...
case "$SLURM_ARRAY_TASK_ID" in
  1)
    python3 main_parallel.py $COMMON_ARGS \
      ...
      --dist_penalty false \
      --storage_dir baseline4_milestone_fitness/recursive_no_drop
    ;;
  2)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_drop_1 \
      ...
      --dist_penalty false \
      --storage_dir baseline4_milestone_fitness/recursive_dropper
    ;;
  3)
    python3 main_parallel.py $COMMON_ARGS \
      --sub ff_drop_1 \
      ...
      --pheromone_receptor true \
      --dist_penalty false \
      --storage_dir baseline4_milestone_fitness/ff_dropper
    ;;
  4)
    python3 main_parallel.py $COMMON_ARGS \
      --sub rec_no_drop_2 \
